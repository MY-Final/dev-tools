...
- 📺 **Danmaku Display** — 全屏展示自定义消息，适合离开工位时留言
  - 🎨 **三种显示模式** — 水平弹幕滚动 / 垂直滚动 / 静止居中大字
  - 🚽 **一键模板** — 10 个预设消息（去洗手间了 / 开会中 / 午休中 等）
  - ⏰ **返回时间** — 支持指定时间（15:30）或相对时间（30 分钟后）
  - ⏳ **实时倒计时** — 全屏自动显示剩余时间，精确到秒，自动更新
  - 🕐 **当前时间** — 全屏右上角显示当前时间（可关闭）
  - 🎛️ **完全自定义** — 字体大小 / 字体颜色 / 背景颜色 / 滚动速度
  - ⚡ **一键全屏** — 单击即可进入全屏展示，点击或 Esc 退出

**改进**
- 🔧 全屏展示改为同窗口覆盖层 + Fullscreen API，不再依赖 window.open
